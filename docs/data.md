...
The storage-side `Dataset` adapter reads the canonical catalog and Parquet files from `page324-data` for collection and maintenance. `operate` captures new responses into a temporary workspace that it creates for that run. During the capture, the adapter also reads the batches in that workspace that are not yet in the R2 catalog. A workspace file never replaces an R2 file. The run uploads the new objects to R2 and then deletes the workspace. Forecast computation receives a separately restored and verified local snapshot.
...
Interactive querying trusts an immutable result or hindcast object once an authoritative mutable pointer selects it. It verifies the canonical snapshot on restore. Deep canonical Parquet reconciliation remains an audit, replay and write concern.
...
Use `--hindcast-versions` on the query skill helper, or `hindcast_versions=` on `open_analysis_session`, to select otherwise: `current` (the default), `none` for a live-only session, `all`, or one or more explicit model version names. Each selection keeps its own session file, so switching between them does not throw away the other. `analysis.session` records `hindcast_version_request` and the `hindcast_model_versions` the session actually contains.
...
`prospective_from` is the first London day on which a live forecast of that model version covered that division. The competition release indexes are the authority for it, so no release metadata can drift away from what the product published. A published forecast covers only kickoffs after it was generated, so no prospective row of a version can fall before that day, and every match hindcast is earlier than it. The analysis bootstrap fails if a match hindcast reaches `prospective_from` or shares a match and model version with `analysis.record_matches`.
